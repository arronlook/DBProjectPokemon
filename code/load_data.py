...
root = findFile('datasets/All_moves.csv')[0]
...
def check_moves(name):
	with open(os.path.join(root, 'Allmoves_parsed.csv'), "r", encoding='utf-8') as source:
...
def parse2():
	...
	rows=[]
	with open(os.path.join(root, 'pokemon.csv'), encoding="utf-8") as f:
		f_csv = csv.reader(f)
		headers = next(f_csv)
		for row in f_csv:
			temp=[]
	...
	rows=[]
	with open(os.path.join(root, 'moves.csv'), "r", encoding='utf-8') as source4:
		reader4 = csv.reader(source4)
		firstline = True
		with open(os.path.join(root, 'pokemon_moves_parsed.csv'), "w", encoding='utf-8', newline='') as result:
	...
	with db_conn.cursor() as cursor:
		with open(findFile('datasets/pokemon_parsed_main.csv')[1], mode='r', encoding='utf-8') as file:
			...
			for row in reader:
				...
				row[-4] = "" if row[-4] is not None and row[-4]=="None" else row[-4]

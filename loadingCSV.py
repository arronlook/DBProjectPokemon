...
if __name__ == "__main__":
	...
	t_pw = "zhangxu123" #password
	db_conn = psycopg2.connect(host=t_host, port=t_port, dbname=t_dbname, user=t_user, password=t_pw)
	db_cursor = db_conn.cursor()
	db_cursor.execute("DROP TABLE IF EXISTS tbl_allMoves")
	db_cursor.execute("CREATE TABLE tbl_allMoves(Name VARCHAR(255) PRIMARY KEY, Type VARCHAR(255),Category VARCHAR(255), Effect VARCHAR(255), Power VARCHAR(255),Acc VARCHAR(255),PP VARCHAR(255),TM VARCHAR(255),Prob VARCHAR(255),Gen INT)" )
